...
export class World {
  ...
  // floor tiles are purely a background layer — placeable on any in-bounds, non-water cell
  // regardless of whether an object already sits there (appliances/furniture still render above)
  placeFloorTile(x, y, type) {
    if (!this.inBounds(x, y) || this.isWater(x, y)) return false;
    this.floorTiles.set(x + ',' + y, type);
    return true;
  }
